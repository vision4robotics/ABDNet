...
def Downsamplingloss(z,z1):
    b,c,w,h=z.size()
    b,c,w1,h1=z1.size()
    z=z.reshape(b*c,w*h)
    z1=z1.reshape(b*c,w1*h1)
    loss=torch.tensor(0)
    for i in range(w1*h1):
      loss1=pow(torch.norm((z1[:, i].unsqueeze(dim=1)-z),dim=0),2).cuda()
      loss2=torch.min(loss1,dim=0)[0].cuda()
      loss=(loss+loss2).cuda()
    lossx=(loss/(b*c*w*h)).sum().cuda()
