...
class Transformer(Module):
    ...
    def forward(self, src: Tensor , srcc: Tensor, src_mask: Optional[Tensor] = None, tgt_mask: Optional[Tensor] = None,
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        self.low_attn = MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward(self, src: Tensor ,srcc: Tensor, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        src2 = self.low_attn(src, srcc, srcc, attn_mask=src_mask,
                               key_padding_mask=src_key_padding_mask)[0]
        src = src + self.dropout1(src2)
        src = self.norm1(src)

        srcc2 = self.high_attn(srcc, srcc, srcc, attn_mask=src_mask,
                               key_padding_mask=src_key_padding_mask)[0]
        srcc = srcc + self.dropout3(srcc2)
        srcc = self.norm3(srcc)
        srcc = self.gap(srcc,src)
        srcc2 = self.linear2(self.dropout(self.activation(self.linear1(srcc))))
        srcc = srcc + self.dropout2(srcc2)
...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, srcc: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None,
        ...
        srcc2 = self.self_attn(srcc, srcc, srcc, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)[0]
        srcc = srcc + self.dropout1(srcc2)
        srcc = self.norm1(srcc)
        
       
        tgt2 = self.mask(srcc,memory,memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        srcc = srcc + self.dropout2(tgt2)
        srcc = self.norm2(srcc)
         
        srcc = self.gap(srcc,memory)
        srcc2 = self.linear2(self.dropout(self.activation(self.linear1(srcc))))
        srcc = srcc + self.dropout3(srcc2)
        srcc = self.norm3(srcc)
        return srcc
