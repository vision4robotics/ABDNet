...
class AnchorTarget():
    ...
    def get(self,bbox,size):
        ...
        # for ii in np.arange(0,size):
        #     for jj in np.arange(0,size):
        #          labelcls2[0,ii,jj]=(((ii-(index2[1]+index2[3])/2)*ran)**2+((jj-(index2[0]+index2[2])/2)*ran)**2)
        
        # se=labelcls2[np.where(labelcls2<=((w//2+h//2)*ran/rcls)**2)]
        
        # labelcls2[np.where(labelcls2<=((w//2+h//2)*ran/rcls)**2)]=1-((se-se.min())/(se.max()-se.min()+1e-4))
        
        # neg=np.where(labelcls2<=0.76)
        # labelcls2[neg]=-2
        
        
        
        
        # negg=np.where(labelcls2==-2)
        # labelcls2[negg]=-1
